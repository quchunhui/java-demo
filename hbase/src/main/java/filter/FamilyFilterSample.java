 ...
 *
 * shell版：
 * scan 'TEST1', {FILTER => "FamilyFilter(=,'binary:df')"}
 * scan 'TEST1', {FILTER => "FamilyFilter(!=,'binary:df')"}
 * scan 'TEST1', {FILTER => "FamilyFilter(=,'substring:df')"}
 */
public class FamilyFilterSample {
    ...
    private static Filter getFilter() {
        return new FamilyFilter(
                ...
                new BinaryComparator(Bytes.toBytes("df")));
    }
}
